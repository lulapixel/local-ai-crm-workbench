...
class TestBuscarComPlacesApi:
    ...
    def test_json_invalido_vira_erro_amigavel(self, tmp_path, monkeypatch):
        class RespostaJsonInvalido(RespostaFake):
            def json(self):
                raise ValueError("HTML inesperado")

        monkeypatch.setattr(
            fontes_maps.requests, "post", lambda *a, **kw: RespostaJsonInvalido(200)
        )

        with pytest.raises(fontes_maps.ErroPlacesApi, match="resposta inválida"):
            fontes_maps.buscar_com_places_api(["x"], tmp_path / "b.csv", "chave")

    def test_resposta_grande_vira_erro_amigavel(self, tmp_path, monkeypatch):
        class RespostaGrande:
            status_code = 200
            headers = {}

            def iter_content(self, chunk_size):
                yield b"x" * (fontes_maps.url_security.MAX_RESPONSE_BYTES + 1)

            def close(self):
                pass

        monkeypatch.setattr(
            fontes_maps.requests, "post", lambda *a, **kw: RespostaGrande()
        )
        with pytest.raises(fontes_maps.ErroPlacesApi, match="resposta grande demais"):
            fontes_maps.buscar_com_places_api(["x"], tmp_path / "b.csv", "chave")

    def test_falha_no_meio_preserva_ultima_captura_e_remove_temporario(self, tmp_path, monkeypatch):
        import requests

        arquivo = tmp_path / "bruto.csv"
        conteudo_anterior = "place_id,title\nantigo,Registro anterior\n"
        arquivo.write_text(conteudo_anterior, encoding="utf-8")
        chamadas = []

        def post_fake(*args, **kwargs):
            chamadas.append(kwargs.get("json") or {})
            if len(chamadas) == 1:
                return RespostaFake(200, {"places": [_place(place_id="novo")]})
            raise requests.RequestException("rede caiu")

        monkeypatch.setattr(fontes_maps.requests, "post", post_fake)

        with pytest.raises(fontes_maps.ErroPlacesApi, match="Não foi possível"):
            fontes_maps.buscar_com_places_api(["primeira", "segunda"], arquivo, "chave")

        assert arquivo.read_text(encoding="utf-8") == conteudo_anterior
        assert list(tmp_path.glob(".bruto.csv.*.tmp")) == []
...
class TestValidarChavePlaces:
    ...
    def test_chamada_places_nao_segue_redirect_automaticamente(self, monkeypatch):
        chamada = {}

        def post_fake(*args, **kwargs):
            chamada.update(kwargs)
            return RespostaFake(302, {})

        monkeypatch.setattr(fontes_maps.requests, "post", post_fake)
        ok, erro = fontes_maps.validar_chave_places("chave-redirect")

        assert ok is False
        assert "Google Places API" in erro
