...
def _salvar_csv_atomico(caminho_saida, campos, linhas):
    """Gera o CSV em temporário e só publica após a escrita completa."""
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            newline="",
            encoding="utf-8-sig",
            dir=str(caminho_saida.parent),
            prefix=f".{caminho_saida.name}.",
            suffix=".tmp",
            delete=False,
        ) as arquivo:
...
# A verificação TLS insegura só pode ser usada em diagnóstico manual e explícito.
# O padrão seguro não envia uma segunda requisição após certificado inválido.
ALLOW_INSECURE_SITE_CHECK_ENV = "PROSPECCAO_ALLOW_INSECURE_SITE_CHECK"
...
def avaliar_site_completo(url):
    ...
    def requisitar(url_alvo, verify=True):
        return url_security.requisitar_url_publica(
            url_alvo,
            requests.get,
    ...
    try:
        ...
        return "ruim", [f"URL rejeitada por segurança ({erro})"], None
    except SSLError:
        problemas.append("certificado de segurança (SSL) inválido ou vencido")
        # Nunca desative TLS silenciosamente para dados vindos do CRM. O modo
        # inseguro é uma exceção manual, útil apenas para diagnóstico local,
        # e continua marcado como problema no resultado.
        permitir_inseguro = os.environ.get(ALLOW_INSECURE_SITE_CHECK_ENV, "").strip().lower() in {
            "1",
            "true",
            "yes",
        }
        if permitir_inseguro:
            try:
                import urllib3
...
def capturar_conteudo_site(url):
    ...
    except (RequestException, url_security.URLNaoPublicaError):
        return None
    if resposta.status_code >= 400:
        return None
...
def processar(caminho_csv_bruto, caminho_queries=CAMINHO_QUERIES_PADRAO, callback_progresso=None,
    ...
    _salvar_csv_atomico(caminho_saida, campos, novos)
