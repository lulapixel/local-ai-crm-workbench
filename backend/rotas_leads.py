...
def obter_lead(place_id):
    """Obtém os detalhes de um lead do Maps pelo seu place_id."""
    if not db.CAMINHO_BANCO.exists():
    ...
    return jsonify(_enriquecer_lead_para_resposta(db.linha_para_dict(linha)))
...
def _validar_busca_por_texto(corpo):
    ...
    queries_bruto = corpo.get("queries", "")
    if isinstance(queries_bruto, list):
        queries_texto = "\n".join(str(q) for q in queries_bruto if q).strip()
    else:
        queries_texto = str(queries_bruto or "").strip()
