...
def mascarar_proxies(valor):
    """Remove credenciais e parâmetros de uma lista de proxies antes de exibir."""
    if not valor:
        return ""

    mascarados = []
    for item in str(valor).replace("\r", "").split("\n"):
        for proxy in item.split(","):
            proxy = proxy.strip()
            if not proxy:
                continue
            try:
                partes = urlsplit(proxy)
                if not partes.scheme or not partes.netloc or not partes.hostname:
                    raise ValueError("proxy inválido")
                host_com_porta = partes.netloc.rsplit("@", 1)[-1]
                if (
                    partes.username is not None
                    or partes.password is not None
                    or "@" in partes.netloc
                ):
                    netloc = f"••••:••••@{host_com_porta}"
                else:
                    netloc = host_com_porta
                mascarados.append(urlunsplit((partes.scheme, netloc, partes.path, "", "")))
            except (ValueError, UnicodeError):
                mascarados.append("••••")
    return ", ".join(mascarados)
...
def obter_proxies_scraper():
    ...
    return jsonify({"configurado": bool(proxies), "proxies": mascarar_proxies(proxies)})
