...
# Local AI CRM Workbench

### Snapshot sanitizado e derivado do ProspectOS para estudo e portfólio
...
> **Origem e escopo:** este é um snapshot derivado e não oficial de
> [nando0x/ProspectOS](https://github.com/nando0x/ProspectOS), preparado para
> demonstrar integrações locais, segurança e testes. O copyright e a licença MIT
> do upstream foram preservados. Consulte [NOTICE.md](NOTICE.md) para separar o
...
```powershell
git clone <URL-DO-SEU-FORK> local-ai-crm-workbench
cd local-ai-crm-workbench
```
...
O arquivo `requirements.txt` mantém as dependências diretas do projeto; o
`requirements-lock.txt` fixa as versões usadas na validação local para tornar
o ambiente reproduzível.
...
> Se o Windows Credential Manager/keyring estiver indisponível, a interface recusa o salvamento (HTTP 503) e **não** grava a chave no banco. Nesse caso, corrija o cofre ou use temporariamente o `.env`; nunca cole segredos em arquivos versionados.
...
│   └── tests/                # suíte de testes (pytest, 460 testes)
└── frontend/
    ├── src/
...
[MIT](LICENSE). O copyright original de Fernando foi preservado. As adaptações
locais e o recorte de portfólio estão descritos em [NOTICE.md](NOTICE.md).

---

## 🙏 Agradecimentos

- [nando0x/ProspectOS](https://github.com/nando0x/ProspectOS): projeto upstream MIT que originou este snapshot derivado
